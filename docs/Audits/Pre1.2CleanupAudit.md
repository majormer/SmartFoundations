...
- Classification: `needs verification`
- Remediation Status: Not started.
- Recommended next action: Inspect Widget Blueprint bindings in Unreal. If legacy widgets still exist in the tree, classify compatibility fields as `do not touch` until the Blueprint is migrated; if they are only stale serialized names, remove fallback paths in a later cleanup.
