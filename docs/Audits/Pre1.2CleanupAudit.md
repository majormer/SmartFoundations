...
- Classification: `needs verification`
- Remediation Status: Not started.
- Recommended next action: Decide whether `ar`/`fa`/`th` are planned supported languages. If not, remove or archive the folders; if yes, add them to localization configs and generate locres in a proper localization pass.
