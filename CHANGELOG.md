...
- **Extend curves its lane belts to match your Belt Routing Mode** - When extending a line of machines along a curve (rotation between copies), the lane belts that run back toward the source (or the previous machine in a Scaled Extend chain) ignored your Belt Routing Mode and always connected straight ("Default") - even with "Curved" selected. Those lane belts now follow the setting (Default / Curve / Straight) in both Extend and Scaled Extend. The short belts inside each machine module are exact copies of the source and are unchanged. (Issue #380)
