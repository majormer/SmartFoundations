...
- **Tap to Toggle Transform Modes (optional, off by default)** - A new Building Behavior setting for controller, Steam Deck, and accessibility players: tap a transform key (Spacing, Steps, Stagger, Rotation, or Recipe on a factory) to switch that mode on, tap it again to switch it off, or tap a different one to change modes - no more holding a key while scrolling. Made for Steam Input radial and touch menus, which send quick taps and cannot hold a key down. The mouse wheel, numpad, HUD, and Smart Panel all work exactly the same in either mode, and turning the setting off restores the classic hold-to-use behavior untouched. Putting the build gun away, switching to a different building, or opening a panel always releases the mode. (Issue #482, for BartekS and The teacher on the Smart! Discord)
- **Scaled factories can now daisy-chain power automatically** - When Upgraded Power Connectors are unlocked, scaling a factory or generator along Smart's X axis can now wire each copy to the next one, matching the common manifold-style layout without needing a separate Extend pass. The persistent default lives under Power Auto-Connect settings, while the Smart Panel can override it for the current build and the HUD reports the effective choice. It respects available connector capacity instead of forcing an invalid cable. (Issue #487, requested by Infarctus on the Smart! Discord)
...
- **Middle-click now respects "Sample Building Copies Settings"** - With the vanilla option off, sampling a configured factory no longer makes Smart copy its production recipe, Power Shards, overclock, Somersloops, or production boost into the new placement. Turning the option back on resumes the full settings copy immediately, while recipes deliberately selected through Smart remain independent. (Issue #489, thanks to michael0meyers)
- **Belt and pipe supports and Smart Walking now default to a foundation-aligned 56 m span** - Smart Walking and support scaling previously stopped short of the maximum straight run. Standard, stackable, ceiling, and wall-mounted belt and pipe supports now derive the correct spacing for a 56 m interval - exactly seven foundations - including wall supports' different scaling axis. Belts and pipes that exceed the vanilla spline limit or form an invalid routed shape are rejected normally. (Issue #488, thanks to MrMaDGaME)
- **Player Relative arrows now follow your view before you scroll** - Holding a scaling or transform control now immediately shows the building axis that Player Relative Controls will affect, and the arrow follows as you turn instead of jumping only after the first wheel input. Forward targets always point forward, lateral targets always point right, and decreasing with the wheel moves against the arrow. The Smart Panel and non-Player-Relative controls keep their existing building-relative arrows. (Issue #486, reported by Danulqua)
- **Extend controls now stay in Chain/Rows terms** - With Player Relative Controls enabled, Extend no longer changes a transform from chain length to parallel rows just because you turned to face another direction. Its runtime HUD and wheel/keybind targets now stay on Chain or Rows, skip the unusable Vertical spacing target, and use the same signed direction for live Extend and restored modules. Switching Extend between the building's left and right sides updates that direction everywhere, so previews, builds, and restored layouts agree. The Smart Panel keeps its absolute X/Y controls. (Issue #478)
- **The Extend HUD no longer counts the building you're extending from** - A plain Extend always read "Extend: 2x1" even though only one new building was being placed, because the counter included the existing source building. It now shows the number of buildings actually going up: "Extend: 1x1" for a single clone, growing only as you scroll to add more. (Issue #491)
- **Extend now copies the sampled recipe to every copy on dedicated servers** - On a dedicated server, extending a sampled building to more than one copy left the extra copies without a production recipe (a single copy worked). The recipe - and sampled Power Shard / Somersloop settings - captured by middle-click now travel with the build to the server, so all copies come out configured the same as in single-player. Respects the vanilla "Sample Building Copies Settings" option: when that is off, nothing is copied anywhere, as intended. (Issue #484, reported by Infarctus on the Smart! Discord)
- **Scaled Extend no longer reports phantom creatures or missing materials** - Extending a building to 3 or more copies could refuse to build with "A creature is in the way!" and "Missing materials!" even with no creature anywhere and materials fully covered (including from the Dimensional Depot). The copies' preview holograms were quietly running the game's own placement checks that Smart's layout management is supposed to replace, and one false flag cascaded into the misleading materials message. Managed preview copies now properly defer their validity to Smart's own placement, topology, and depot-aware material checks - which still block building for real reasons. (Issue #470, reported by guilhermemagro)
- **Extend now releases predictably on both sides of a machine row** - Brushing over or middle-click sampling a matching building engages Extend transiently: move the crosshair away and it lets go, on either end of a manifold row (one side used to silently stick, leaving a locked hologram that ignored the Hold key). Persistence now follows intent exactly: scaling beyond a single copy keeps the layout while you look around - and scaling back down to one returns to the transient behavior instead of staying stuck - while the Hold key (H) pins an unscaled Extend so you can inspect it from any angle, and pressing it again unpins. (Issue #478)
- **Batch upgrades now charge the real cost of belts, pipes, and lifts** - The Smart Upgrade panel and its upgrade execution priced spline buildables as if every belt were a single short segment: a long Mk.2 belt line upgraded to Mk.6 could cost a fraction of its true material price while still refunding the old belts in full - effectively free materials. Both the panel's estimated cost and the actual charge now use the exact length-based cost the game itself uses, so the estimate you see is exactly what you pay, and refunds match what was actually replaced. Upgrades are also settled transactionally now: if a replacement fails to build, the charged materials are returned and no refund is granted, instead of the old behavior where a failure could keep both. (Issue #485, reported by MrMaDGaME on the Smart! Discord)
